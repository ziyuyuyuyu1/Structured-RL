...
for SUB_EXPERIMENT_NAME in "${SUB_EXPERIMENT_NAMES[@]}"; do
    for GLOBAL_STEP in "${GLOBAL_STEP_LIST[@]}"; do
        ...
        MODEL_NAME_OR_PATH=${CHECKPOINTS_DIR}/${PROJECT_NAME}/${SUB_EXPERIMENT_NAME}/global_step_${GLOBAL_STEP}
        OUTPUT_DIR=${CHECKPOINTS_DIR}/${PROJECT_NAME}/${SUB_EXPERIMENT_NAME}/logic_sentences/global_step_${GLOBAL_STEP}
...
for FULL_EXPERIMENT_NAME in "${FULL_EXPERIMENT_NAMES[@]}"; do
    for GLOBAL_STEP in "${GLOBAL_STEP_LIST[@]}"; do
        echo "======== Evaluating checkpoint at global step: ${GLOBAL_STEP} ========"
        MODEL_NAME_OR_PATH=${CHECKPOINTS_DIR}/${PROJECT_NAME}/${FULL_EXPERIMENT_NAME}/global_step_${GLOBAL_STEP}
        OUTPUT_DIR=${CHECKPOINTS_DIR}/${PROJECT_NAME}/${FULL_EXPERIMENT_NAME}/logic_sentences/global_step_${GLOBAL_STEP}
        bash sh/generate_logic_sentences.sh $PROMPT_TYPE $MODEL_NAME_OR_PATH $MAX_TOKENS $OUTPUT_DIR
    done
done